...
void main()
{
	...
	vec2 totalDistortion = (texture(dudvMap, distortedTexCoords).rg * 2.0 - 1.0) * waveStrength * clamp(waterDepth/0.8, 0.0, 1.0);
    ...
    refractColour = mix(refractColour, vec4(0.0, 0.15, 0.0, 1.0), clamp(waterDepth/2.0, 0.0, 1.0));

    float waterAlphaTransitionDepth = 0.25;

    vec3 reflectedLight = reflect(normalize(fromLightVector), normal);
	float specular = max(dot(reflectedLight, viewVector), 0.0);
	specular = pow(specular, shineDamper);
	vec3 specularHighlights = lightColour * specular * reflectivity * clamp(waterDepth/waterAlphaTransitionDepth, 0.0, 1.0);
    ...
    out_Colour.a = clamp(waterDepth/waterAlphaTransitionDepth, 0.0, 1.0);
}